...
//answers to quiz will be tallied in this object
podcasts.genres = {
  horror: 0,
  comedy: 0,
  trueCrime: 0,
...
// function will go through list of podcasts and will only append to p is the podcast matches highestGenre and the filter which is the answer to the final question
podcasts.test = function() {
    ...
    for (let key in podcasts.entry[i]) {
      if (podcasts.entry[i][key] === podcasts.highestGenre) 
      {
       if (podcasts.entry[i].length === podcasts.filter){    
        ...
        <p>Link: <a href="${podcasts.entry[i].link}">${podcasts.entry[i].link}</a>
        </div>
  ...
  //main function used to tally answers from the user
...
// array of podcasts. Answers will be taken from here
podcasts.entry = [
  {
...

  {
    ...
    length: "short",
    img: "assets/ad.png",
    link: "http://www.nightvalepresents.com/aliceisntdead/"
  }
];
